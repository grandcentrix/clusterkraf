...
class ClusterTransitionsAnimation implements Animator.AnimatorListener,
    ...
    private final HashMap<Marker, AnimatedTransition> animatedTransitionsByMarker
            = new HashMap<Marker, AnimatedTransition>();

    private final HashMap<Marker, ClusterPoint> stationaryTransitionsByMarker
    ...
    /**
     * Add temporary stationary and animated transition markers, holds onto references, and calls
     * back to the Host when finished
     *
    ...
    /**
     * The Host must call this after it plots its cluster points so that the stationary and animated
     * transition markers can be removed.
     */
        ...
        private LatLng[] getPositions() {
            ...
            for (AnimatedTransition transition : transitions) {
                ...
                if (transition.spans180Meridian() == false) {
                     ...
                     */
                    double shiftedStartLon = start.longitude < 0 ? start.longitude + 360
                            : start.longitude;
                    double shiftedEndLon = end.longitude < 0 ? end.longitude + 360 : end.longitude;
                    double shiftedCurrentLon = shiftedStartLon + (value * (shiftedEndLon
                            - shiftedStartLon));
