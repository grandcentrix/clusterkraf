...
public class Clusterkraf {
    ...
    private static class InnerCallbackListener
            implements ClusteringOnCameraChangeListener.Host, ClusterTransitionsAnimation.Host,
        ...
        private final boolean cluterOnCameraChange;
        ...
        private InnerCallbackListener(Clusterkraf clusterkraf,
            ...
            clusteringOnCameraChangeListener = new ClusteringOnCameraChangeListener(this,
                    clusterkraf.options);

            cluterOnCameraChange = clusterkraf.options.isEnableClusteringOnCameraChange();
        }
        ...
        public View getInfoContents(Marker marker) {
            ...
            if (clusterkraf != null && clusterkraf.currentClusterPointsByMarker != null
                ...
                InfoWindowDownstreamAdapter infoWindowDownstreamAdapter = clusterkraf.options
                        .getInfoWindowDownstreamAdapter();
        ...
        public void onClusterTransitionFinished() {
            ...
            //mapClickListener.onMapClick(null);
        }
        ...
        public void onInfoWindowClick(Marker marker) {
            ...
            if (clusterkraf != null && clusterkraf.currentClusterPointsByMarker != null
                ...
                ClusterPoint clusterPoint = clusterkraf.currentClusterPointsByMarker.get(marker);
                OnInfoWindowClickDownstreamListener downstreamListener = clusterkraf.options
        ...
        public boolean onMarkerClick(Marker marker) {
            ...
            if (clusterkraf != null && clusterkraf.currentClusterPointsByMarker != null
                ...
                if (clusterPoint == null) {
                    if (clusterkraf.transitionsAnimation.getAnimatedDestinationClusterPoint(marker)
                            != null) {
                        exempt = true;
                        // animated marker click is not supported
                    } else {
                        clusterPoint = clusterkraf.transitionsAnimation.getStationaryClusterPoint(
                                marker);
                    }
                }
                OnMarkerClickDownstreamListener downstreamListener = clusterkraf.options
                        .getOnMarkerClickDownstreamListener();
    ...
    private class ClusterTransitionsBuildingTaskHost
        ...
        public void executeTask(Projection projection) {
            if (projection != null) {
                ClusterTransitionsBuildingTask.Argument arg
                        = new ClusterTransitionsBuildingTask.Argument();
        ...
        @Override
        public void onClusterTransitionsBuildingTaskPostExecute(
                ClusterTransitionsBuildingTask.Result result) {
            ProcessingListener processingListener = options.getProcessingListener();
            if (processingListener != null) {
                processingListener.onClusteringFinished();
            }
    ...
    private HashMap<Marker, ClusterPoint> currentClusterPointsByMarker
            = new HashMap<Marker, ClusterPoint>();
    ...
    /**
     * Construct a Clusterkraf instance to manage your map with customized options and a list of
     * points
     *
    ...
    public Clusterkraf(GoogleMap map, Options options, ArrayList<InputPoint> points,
        ...
        this.transitionsAnimation = new ClusterTransitionsAnimation(map, options,
                innerCallbackListener);
    ...
    /**
     * Remove all existing InputPoint objects and add a new list of InputPoint objects for
     * clustering
     *
    ...
    public void showInfoWindow(Marker marker, ClusterPoint clusterPoint) {
        ...
        if (map != null && marker != null && clusterPoint != null) {
            long dirtyUntil = System.currentTimeMillis() + options
            ...
            map.animateCamera(cameraUpdate, options.getShowInfoWindowAnimationDuration(),
                    new CancelableCallback() {
                        ...
                        public void onFinish() {
                            innerCallbackListener.handler.post(new Runnable() {
                                ...
                                @Override
                                public void run() {
                                    innerCallbackListener.clusteringOnCameraChangeListener
                                            .setDirty(0);
                                }
                            });
                        }
    ...
    /**
     * Animate the camera so all of the InputPoint objects represented by the passed ClusterPoint
     * are in view
     */
    public void zoomToBounds(ClusterPoint clusterPoint) {
        GoogleMap map = mapRef.get();
        if (map != null && clusterPoint != null) {
            innerCallbackListener.clusteringOnCameraChangeListener.setDirty(
                    System.currentTimeMillis());
            CameraUpdate cameraUpdate = CameraUpdateFactory.newLatLngBounds(
                    clusterPoint.getBoundsOfInputPoints(), options.getZoomToBoundsPadding());
